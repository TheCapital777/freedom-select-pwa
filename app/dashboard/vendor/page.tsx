      ...
      <div style={{ padding: "24px 20px 20px", borderBottom: "1px solid #1E1E1E" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "16px", marginBottom: "20px" }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <p style={{ fontSize: "13px", color: "#555" }}>📍 {vendor.location}</p>
          </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "10px" }}>
          {[
            ...
            { label: "Rating",   val: `⭐ ${vendor.rating}` },
          ].map((k) => (
            <div key={k.label} style={{ ...card, padding: "14px 12px", textAlign: "center" }}>
              <p style={{ fontWeight: 900, fontSize: "20px", color: "#FFBB1C", marginBottom: "6px" }}>{k.val}</p>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
            {products.map((p, i) => (
              <motion.div key={p.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
                ...
                <div style={{ width: "56px", height: "56px", borderRadius: "10px", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "28px", flexShrink: 0, background: "#0C0C0C", border: "1px solid #2A2A2A" }}>{p.emoji}</div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                    ...
                    {p.featured && <span style={{ padding: "4px 10px", borderRadius: "5px", background: "#FFBB1C", color: "#0C0C0C", fontSize: "11px", fontWeight: 800 }}>★ Featured</span>}
                  </div>
                </div>
              </motion.div>
            ))}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
            {orders.length === 0 ? (
              <div style={{ ...card, padding: "48px 24px", textAlign: "center" }}>
                ...
                <p style={{ fontSize: "16px", color: "#555" }}>No orders yet</p>
              </div>
            ) : orders.map((o, i) => {
              ...
              return (
                <motion.div key={o.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
                  ...
                  <div style={{ padding: "18px 18px" }}>
                    {myItems.map((it) => (
                      <p key={it.product_id} style={{ fontSize: "14px", color: "#888", marginBottom: "8px" }}>
                        → Pack: <span style={{ color: "#F0F0F0", fontWeight: 700 }}>{it.product_name} ×{it.qty} {it.unit}</span>
                      </p>
                    ))}
                    <p style={{ fontSize: "13px", color: "#555", marginBottom: "16px" }}>📍 Deliver to: {o.delivery_address}</p>
                    {o.status === "confirmed" && (
        ...
        {tab === "Wallet" && (
          <div>
            <div style={{ ...card, padding: "28px 20px", marginBottom: "20px", textAlign: "center", background: "linear-gradient(135deg, #181208, #141414)", border: "1px solid rgba(255,187,28,0.25)", position: "relative", overflow: "hidden" }}>
              ...
              <p style={{ fontSize: "11px", fontWeight: 700, letterSpacing: "0.18em", textTransform: "uppercase", color: "#555", marginBottom: "12px" }}>Available Balance</p>
              ...
              {getOrdersByVendor(vendor.id).map((o, i, arr) => (
                <div key={o.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "14px 0", borderBottom: i < arr.length - 1 ? "1px solid #1A1A1A" : "none" }}>
                  <div>
                    ...
                    <p style={{ fontSize: "12px", color: "#555" }}>{new Date(o.created_at).toLocaleDateString()}</p>
                  </div>
